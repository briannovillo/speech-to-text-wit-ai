...
while [ $i -le $end ]; do
    ...
    python -c "import time; print time.strftime('%H:%M:%S', time.gmtime(${i-10}))+' --> '+time.strftime('%H:%M:%S', time.gmtime($(($i+10))))" >> videoplayback.srt # print time start and end with format: 00:00:00 --> 00:00:10
